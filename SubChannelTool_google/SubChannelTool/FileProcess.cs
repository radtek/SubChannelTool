...
namespace SubChannelTool
{
    public class FileProcess
    {
        ...
        // 替换filePath文件中的fromValue为toValue
        public static void ContentReplace(string filePath, string fromValue, string toValue)
        {
            if (System.IO.File.Exists(filePath))
            {
                string data = fileToString(filePath);
                if (data.Contains(fromValue)) data = data.Replace(fromValue, toValue);
